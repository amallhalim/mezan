...
export default function MacroNutrientStats({ protein, carbs, fat, isCompact = false }: MacroNutrientStatsProps) {
  ...
  const macros = [
    { label: 'Protein', value: protein, color: 'var(--protein)', icon: <Zap className="size-3" />, percentage: (protein / total) * 100 },
    { label: 'Carbs', value: carbs, color: 'var(--carbs)', icon: <Flame className="size-3" />, percentage: (carbs / total) * 100 },
    { label: 'Fat', value: fat, color: 'var(--fat)', icon: <Droplets className="size-3" />, percentage: (fat / total) * 100 },
  ];
    ...
    <div className={`grid grid-cols-3 gap-2 ${isCompact ? 'mb-4' : 'mb-6'}`}>
      {macros.map((macro, i) => (
        <div key={i} className="bg-white/5 rounded-2xl p-2.5 border border-white/5 flex flex-col items-center justify-center group hover:bg-white/10 transition-all">
          <div className="flex items-center gap-1 mb-1.5" style={{ color: macro.color }}>
            {macro.icon}
