...
import { X, Zap, Share2, Heart, Send, MessageSquare, Copy, Flame, Award } from 'lucide-react';
...
export default function ResultModal({ item, onClose }: ResultModalProps) {
  ...
  const shareOptions = [
    { name: 'X', icon: <Send className="size-4" />, color: 'bg-white/5 text-white' },
    { name: 'WA', icon: <MessageSquare className="size-4" />, color: 'bg-[#25D366]/10 text-[#25D366]' },
    { name: 'Copy', icon: <Copy className="size-4" />, color: 'bg-white/5 text-white' },
  ];

  const getBadges = () => {
    const badges = [];
    if (item.protein > 20) badges.push({ text: "High Protein", icon: <Award className="size-3" />, color: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" });
    if (item.calories < 150) badges.push({ text: "Light Option", icon: <Flame className="size-3" />, color: "bg-blue-500/10 text-blue-400 border-blue-500/20" });
    if (item.carbs > 50) badges.push({ text: "Energy Rich", icon: <Zap className="size-3" />, color: "bg-amber-500/10 text-amber-400 border-amber-500/20" });
    return badges;
  };
      ...
      <div className="relative bg-[#09090b] w-full max-w-[440px] rounded-[3.5rem] border border-white/10 shadow-[0_0_100px_rgba(16,185,129,0.1)] overflow-hidden animate-in zoom-in-95 duration-500">
        ...
        <div className="p-8 pt-16 space-y-8">
          ...
          <div className="text-center space-y-2">
            <h2 className="text-4xl font-black text-white tracking-tighter leading-none">{item.name}</h2>
            <div className="flex items-center justify-center gap-3">
               <span className="h-px w-8 bg-gradient-to-r from-transparent to-primary/40" />
               <p className="text-primary font-black text-xl" dir="rtl">{item.nameAr}</p>
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-4">
              {getBadges().map((badge, i) => (
                <div key={i} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${badge.color}`}>
                  {badge.icon}
                  {badge.text}
                </div>
              ))}
            ...
            <div className="grid grid-cols-2 gap-8 items-center">
              <div className="space-y-6">
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Total Energy</p>
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-xs font-black text-gray-600 uppercase tracking-widest">kcal</span>
                  </div>
                </div>

                <div className="space-y-4">
                  {[
                    { label: 'Protein', value: item.protein, color: 'var(--protein)' },
                    { label: 'Carbs', value: item.carbs, color: 'var(--carbs)' },
                    ...
                    <div key={m.label} className="flex items-center justify-between group">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest group-hover:text-white transition-colors">{m.label}</span>
                      </div>
                      <span className="text-sm font-black tabular-nums" style={{ color: m.color }}>{m.value}g</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="flex gap-4">
            ...
            >
              ...
              <span className="text-sm tracking-widest font-black uppercase">{isSaved ? 'SAVED!' : 'SAVE TO DIARY'}</span>
            </button>
          </div>
          ...
          <div className="flex justify-center gap-4 py-2">
            {shareOptions.map((opt) => (
              <button key={opt.name} className={`${opt.color} size-12 rounded-2xl border border-transparent hover:border-white/10 flex items-center justify-center transition-all hover:-translate-y-1 active:scale-95 group`}>
