...
- _Zustand_: Overkill since no other page needs the calculator's current plate state.
- _React Context_: Would be useful if prop drilling is severe. Since the calculator page just passes state down one level to layout components, prop drilling is minimal. Returning the state and dispatch functions from a custom hook is sufficient.
...
- _Custom CSS classes_: Tailwind provides enough utility for radial gradients (`bg-[radial-gradient(...)]`) or custom config extensions, avoiding the need for separate `.css` files.
